...
PROFILE = PROFILES["positive-reward"]
...
def derive_reward_diagnostics(
    ...
    for task in TASK_ORDER:
        ...
        task_chunks = [
            chunk for episode in task_episodes for chunk in episode["chunks"]
        ]
            ...
            all_episode_rows.append(
                {
                    ...
                    "initial_return": float(
                        episode.get("discounted_returns", [0.0])[0]
                    ),
                }
            )
        ...
        per_task.append(
            {
                ...
                "first_5_memory_selection_rate": rate(
                    [
                        chunk.get("selected_source") == "memory"
                        for chunk in first_5_chunks
                    ]
                ),
                "final_10_memory_selection_rate": rate(
                    [
                        chunk.get("selected_source") == "memory"
                        for chunk in final_10_chunks
                    ]
                ),
            }
        )
    ...
    for source in ("generator", "memory"):
        ...
        source_stats[source] = {
            ...
            "mean_score": mean(chunk["evaluator_score"] for chunk in source_chunks),
...
def plot_evaluator_scores(metrics: dict[str, dict[str, dict]]) -> None:
    ...
    ax.legend(
        handles + second_handles, labels + second_labels, ncol=3, loc="upper center"
    )
