...
from jitpi05.config import BIAS_PHRASE, BIAS_VALUE, MAX_PLAN_TOKENS, QWEN_ID, TOP_K
from jitpi05.datasets import tensor_to_pil
...
class InspectableLogitsProcessor(LogitsProcessor):
    ...
    def __call__(self, input_ids: torch.Tensor, scores: torch.Tensor) -> torch.Tensor:
        ...
        self.modified_logits.append(
            modified.detach().to(device="cpu", dtype=torch.bfloat16)
        )
        self.steps.append(
            {
                "step": step,
...
def make_phrase_bias(
    tokenizer, phrase: str, bias: float
) -> tuple[LogitsModifier, list[int]]:
    ...
    def modifier(
        step: int, _input_ids: torch.Tensor, logits: torch.Tensor
    ) -> torch.Tensor:
...
def generate_plan_pair(
    ...
    modifier, target_ids = make_phrase_bias(
        processor.tokenizer, bias_phrase, bias_value
    )
