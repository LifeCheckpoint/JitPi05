...
def test_augmented_candidates_center_logits_and_deduplicate() -> None:
    result = build_augmented_candidates(
        [
            ...
            {
                "text": "move toward left plate",
                "semantic_key": "move|held-object|left-plate",
            },
        ],
...
def test_positive_only_evaluator_schema_and_parser() -> None:
    ...
    for invalid in (
        ...
        '{"result":"no effect","usefulness":"useful","certainty":"certain","score":0}',
    ):
        try:
            ...
            raise AssertionError(
                "strict positive-only parser must reject invalid reward"
            )
    ...
    proposal = VisualProposal.model_validate(
        {
            ...
            "candidates": [
                {
                    "text": "move toward the object in the center",
                    "semantic_key": "move|gripper|center",
                },
                {"text": "move slightly left", "semantic_key": "move|gripper|left"},
                {"text": "lower the gripper", "semantic_key": "lower|gripper|center"},
                {
                    "text": "close around the nearby cup",
                    "semantic_key": "grasp|nearby-cup|gripper",
                },
                {
                    "text": "carry the held object toward the left plate",
                    "semantic_key": "move|held-object|left-plate",
                },
            ],
        }
...
def test_five_task_positive_only_experiment_configuration() -> None:
    ...
    assert JITRL_OUTPUT_DIR.as_posix() == (
...
def test_multitask_summary_keeps_task_pairs_and_macro_average() -> None:
    ...
    assert (
        summary["tasks"][tasks[0]["name"]]["paired_differences"]["success_rate"]["mean"]
        == 1.0
    )
    assert summary["task_macro"]["methods"]["jitrl"]["success_rate"]["mean"] == 0.5
    assert summary["task_macro"]["paired_differences"]["success_rate"]["mean"] == 0.5
