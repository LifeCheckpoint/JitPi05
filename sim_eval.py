    ...
    try:
        for task_spec in SIM_TASKS:
            ...
            for episode_index in range(SIM_EPISODES):
                envs, env, env_preprocessor, _ = make_single_env(task_spec, episode_index)
                try:
...
def rollout_condition(
    ...
    envs, env, env_preprocessor, env_postprocessor = make_single_env(task_spec, episode_index)
    episode_seed = SEED + episode_index
    reset_rollout_state(policy, preprocessor, postprocessor, episode_seed)

    task = condition
    video_path = SIM_OUTPUT_DIR / "videos" / task_spec["name"] / condition_name / f"episode_{episode_index}.mp4"
    ...
    try:
        ...
        while len(actions) < max_steps and not success:
            ...
            for action in action_chunk[:SIM_ACTION_STEPS]:
                ...
                observation, reward, terminated, truncated, info = env.step(env_action.numpy())
    ...
    return {
        ...
        "action_chunks": torch.stack(action_chunks) if action_chunks else torch.empty(0, 50, 7),
        "rewards": torch.tensor(rewards, dtype=torch.float32),
        "video_path": str(video_path),
    }


def run_simulation_evaluation(plans: dict) -> dict:
    ...
    try:
        for task_spec in SIM_TASKS:
            ...
            for episode_index, plan in enumerate(plans[task_name]):
                ...
                conditions = {
                    "original_task": task,
                    "oracle_subtask": conditioned_task(task, task_spec["oracle_subtask"]),
                    "qwen_subtask": conditioned_task(task, plan["plain"]["text"]),
                    "modulated_qwen_subtask": conditioned_task(task, plan["modulated"]["text"]),
                }
    ...
    for task_spec in SIM_TASKS:
        ...
        summary["tasks"][task_name] = {
            ...
            "conditions": {
                name: {
                    "successes": [
                        episode["conditions"][name]["success"] for episode in task_results
                    ],
                    "success_rate": sum(
                        episode["conditions"][name]["success"] for episode in task_results
                    )
                    ...
                    / len(task_results),
                    "mean_sum_reward": sum(
                        episode["conditions"][name]["sum_reward"] for episode in task_results
                    )
                    / len(task_results),
                    "videos": [
                        episode["conditions"][name]["video_path"] for episode in task_results
                    ],
                }
...
def save_simulation_artifacts(plans: dict, results: dict) -> dict:
    ...
    torch.save(planning_tensors, SIM_OUTPUT_DIR / "planning_logits.pt")
    torch.save(rollout_tensors, SIM_OUTPUT_DIR / "rollouts.pt")

    summary = summarize(plans, results)
    (SIM_OUTPUT_DIR / "summary.json").write_text(
