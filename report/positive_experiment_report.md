...
> 报告生成日期：2026-07-30
> 
> 本报告对应的图表与数据位于 [`positive_figures/`](positive_figures/) 和 [`positive_tables/`](positive_tables/)。上一轮 signed-reward 实验报告仍保留在 [`experiment_report.md`](experiment_report.md)。
...
实现位置包括 [`settings.py`](../settings.py)、[`jitrl_eval.py`](../jitrl_eval.py)、[`jitrl_planner.py`](../jitrl_planner.py)、[`gemini_vlm.py`](../gemini_vlm.py) 和 [`eval_jitrl.py`](../eval_jitrl.py)。
...
成功 episode 的最后一个 chunk 额外增加 `+1`；失败 episode 不加入 terminal negative penalty；之后使用 `γ=0.95` 计算 discounted returns。实现见 [`_evaluate_episode_chunks()`](../jitrl_eval.py#L348)。
...
![各任务成功率](positive_figures/fig01_success_rate_by_task.png)
...
![在线成功率曲线](positive_figures/fig03_online_success_curves.png)
...
![Memory 机制指标](positive_figures/fig04_memory_mechanism_metrics.png)
...
![Strict positive-only evaluator](positive_figures/fig05_evaluator_score_distribution.png)
...
![Task 69 案例](positive_figures/fig08_case_task69_episode4.jpg)
...
![Task 79 reward trace](positive_figures/fig06_task79_episode9_reward_trace.png)
...
![Task 59 成功案例](positive_figures/fig07_case_task59_episode13.jpg)
...
![Task 69 失败案例](positive_figures/fig08_case_task69_episode4.jpg)
...
![Task 79 失败案例](positive_figures/fig09_case_task79_episode9.jpg)
